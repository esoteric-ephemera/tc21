...
import settings
from ballone_s_q_data.reader import convert_and_write
...
def plot_qmc_sq_dat():
    ...
    for adir in os.listdir('./ballone_s_q_data/data_files/'):
        if adir[0]=='.':
            continue
    ...
    for idir,adir in enumerate(to_plot):
        fl_l = glob('./ballone_s_q_data/data_files/'+adir+'/sk_*.csv')
        if len(fl_l) == 0:
            convert_and_write()
        for ifl,fl in enumerate(fl_l):
            ...
            if smoothing == 'GN':
                ...
                for i in range(sk.shape[0]):
                    ...
                    llen = mmax - mmin + 1
                    skt = sk[mmin:mmax+1]
                    mean = np.sum(skt)/llen
                    var = np.sum(skt**2)/llen - mean**2
