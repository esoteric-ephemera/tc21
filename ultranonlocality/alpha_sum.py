...
from settings import crystal,pi,Eh_to_eV,LDA,nproc
from dft.mcp07 import mcp07_dynamic
from dft.gki_fxc import gki_dynamic_real_freq
from dft.qv_fxc import fxc_longitudinal as qv_fxc
from ultranonlocality.pseudo_pt import get_n_g
...
def wrap_kernel(q,omega,n,wfxc):
    ...
    if wfxc == 'MCP07':
        ...
        fxc=gki_dynamic_real_freq(dv,omega,x_only=False,revised=True,param='PW92',dimensionless=False)
    elif wfxc == 'QV':
        fxc = qv_fxc(dv,omega)
    elif wfxc == 'TC':
        fxc = mcp07_dynamic(q,omega,dv,axis='real',revised=True,param=LDA)
    else:
...
def calc_alpha(fxcl):
    ...
    for fxc in fxcl:
        ...
        ofl = wdir+crystal+'_alpha_omega_'+fxc+addn+'.csv'
        intwg = 1.0/3.0
        if fxc in dyn_only_regex and ncore > 1:
            pool = mp.Pool(processes=ncore)
...
def alpha_plotter(fxcl,sign_conv=1):
    ...
    for ifxc,anfxc in enumerate(fxcl):
        flnm = wdir+crystal+'_alpha_omega_'+anfxc+addn+'.csv'
        ...
        ax[0].plot(om[anfxc],alp_re[anfxc],color=clist[ifxc],linestyle=line_styles[ifxc%len(line_styles)],linewidth=2,label=lbl)
    ...
    if crystal in semicond_regex:
        ...
        plt.suptitle('{:}'.format(crystal),fontsize=16)
    for ifxc,anfxc in enumerate(fxcl):
...
if __name__=="__main__":
    ...
    alpha_plotter(['MCP07','MCP07_k0','DLDA','QV'],sign_conv=-1)
