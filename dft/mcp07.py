...
def mcp07_dynamic(q,omega,dv,axis='real',revised=False,pars={},param='PZ81'):
    ...
    if revised:
        if len(pars) > 0:
            fp = pars
        elif len(pars) == 0 and len(settings.rMCP07_pars) >0:
            fp = settings.rMCP07_pars
        else:
        ...
        fxc_omega = gki_dynamic(dv,F2*omega,axis=axis,revised=revised,param=param,use_par=settings.gki_param)
        fxc = (1.0 + np.exp(-(q/kscr)**2)*(fxc_omega/f0 - 1.0))*fxc_q
    else:
        fxc_omega = gki_dynamic(dv,omega,axis=axis,revised=revised,param=param,use_par=settings.gki_param)
