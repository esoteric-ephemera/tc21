...
def get_inf_bounds(method,wbound,bds,targf=None,itgr=None,max_inc=200):
        ...
        for ival in range(3):
            for inc in range(max_inc):
                ...
                if method == 'fun_and_deriv':
                    ...
                    prog_cond = abs(tval)<10.0**(-4-ival)*relval
                if prog_cond:
                    if ival < 2:
                        if wbound == 'lower':
...
def global_adap(fun,bds,opt_d,args=(),kwargs={}):
    ...
    max_recur = ceil((np.log(abs(bds[1]-bds[0])) + 12*np.log(10.0))/np.log(2.0)) # 2**max_recur bisections yields a region of width 10**(-10)
    ...
    for irecur in range(max_recur):
        ...
        if opt_d['error monitoring']:
            print(np.sum(err_l),np.sum(sum_l))
        if np.sum(err_l) != np.sum(err_l): # if the output is NaN, completely failed
            return np.sum(sum_l),{'code':0,'error':np.sum(err_l)}
        if np.sum(err_l) < prec:
            return np.sum(sum_l),{'code':1,'error':np.sum(err_l)}
        else:
            ...
            if opt_d['itgr'] == 'CC' and np.sum(err_l) < 0.8*prec:
                ...
                for it in range(0,2):
                    ...
                    err = abs(tsum-terr)
                    if np.sum(err_l)+err < prec:
                        return np.sum(sum_l)+tsum,{'code':1,'error':np.sum(err_l)+err}
                    else:
                        if err > bad_err:
                            break
                        else:
    ...
    for iter in range(max_depth+1):
        ...
        for sum_fac in [-1,1]:
            ...
            for m in range(lbd,max_m+1,step):
                ...
                if abs(tint - oint) < prec*abs(oint):
                    tsum += tint
                    m_conv = True
                    break
                else:
...
def adap_cc(fun,bds,opt_d,args=(),kwargs={}):
    ...
    for iter in range(max_depth):
        ...
        if abs(osum - tsum) < prec*abs(osum) and iter >= min_depth:
...
if __name__ == "__main__":
    ...
    def bpp(t):
        y = t/(1.0 - t**2)
        dy = (1.0 + t**2)/(1.0 - t**2)**2
        r = 1.0 - y*np.arctan2(1.0,y)
        g = -4/(2*np.pi)**3*y**2*(3.0-y**2)/(1.0 + y**2)**3
        return -(2*np.pi)**2*np.log(r)*g*dy

    def bppp(t):
        y = t/(1.0 - t**2)
        dy = (1.0 + t**2)/(1.0 - t**2)**2
        r = 1.0 - y*np.arctan2(1.0,y)
        return 2.0/(9*np.pi)*(2.0 + 1.0/(1.0 + y**2))*(1.0/(1.0 + y**2)**2/r)*dy

    bp2,bp2_err = nquad(bpp,(0.0,1.0),'global_adap',{'itgr':'GK','prec':1.e-8,'npts':21})
    bp3,bp3_err = nquad(bppp,(0.0,1.0),'global_adap',{'itgr':'GK','prec':1.e-8,'npts':21})

    bc = (5.0/9.0+bp2+bp3)/(3.0*pi**2)
    print(bc)
    ...
    print('Double-exponential/Tanh-Sinh',dbl_exp(sig_x,(0.0,1.0)))
    print('Trapezoid',trap(sig_x,(1.e-6,1.0-1.e-6),prec=1.e-6))
    print('Simpson',trap(sig_x,(1.e-6,1.0-1.e-6),prec=1.e-6,simpson=True))
    print('Global-adaptive Gauss-Kronrod',global_adap(sig_x,(0.0,1.0),{'itgr':'GK'}))
    print('Global-adaptive Clenshaw-Curtis',global_adap(sig_x,(1.e-6,1.0-1.e-6),{'itgr':'CC'}))
    print('Adaptive Clenshaw-Curtis',adap_cc(sig_x,(1.e-6,1.0-1.e-6)))
    exit()
    """
